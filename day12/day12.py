...
def usage():
    ...
    print("Find the fewest steps required to move from your current position to the location that should get the best signal.\n")
...
def main(input_file_name):
    ...
    start_node = next(filter(lambda n: n.isStart, graph))
    dest_node = next(filter(lambda n: n.isDest, graph))
    path = find_shortest_path(graph, start_node, dest_node)
    flattened_path = list(flattenNthArray(path))
    fewest_required_steps = len(flattened_path) - 1
